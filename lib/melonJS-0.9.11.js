...
(function ($) {
	...
	me.Rect = Object.extend(
			/** @scope me.Rect.prototype */
		{
			 ...
			 * @return {me.Vector2d}
			 */
			collideWithRectangle : function (/** {me.Rect} */
				rect) {
				// response vector
			...
			/** @scope me.SpriteObject.prototype */
		{
			...
			draw : function (context) {
				...
				if ((this.scaleFlag) || (angle !== 0)) {
					...
					var ax = w * this.anchorPoint.x,
					ay = h * this.anchorPoint.y;
	...
	me.Viewport = me.Rect.extend(
			/** @scope me.Viewport.prototype */
		{
			...
			fadeOut : function (color, duration, onComplete) {
				...
				this._fadeOut.tween = me.entityPool.newInstanceOf("me.Tween", this._fadeOut).to({
						alpha : 0.0
					}, this._fadeOut.duration).onComplete(onComplete || null);
			...
			fadeIn : function (color, duration, onComplete) {
				...
				this._fadeIn.tween = me.entityPool.newInstanceOf("me.Tween", this._fadeIn).to({
						alpha : 1.0
					}, this._fadeIn.duration).onComplete(onComplete || null);
...
(function ($) {
	...
	me.video = (function () {
		...
		api.init = function (wrapperid, game_width, game_height, doublebuffering, scale, aspectRatio) {
			...
			window.addEventListener('resize', throttle(100, false, function (event) {
		...
		function enablePointerEvent() {
			if (!pointerInitialized) {
				...
				if (obj.throttlingInterval < 17) {
					...
					me.video.getScreenCanvas().addEventListener(activeEventList[POINTER_MOVE], throttle(obj.throttlingInterval, false, function (e) {
							onMoveEvent(e);
						}), false);
				}
	...
	me.Tween.Easing = {
		...
		Bounce : {
			...
			InOut : function (k) {
				...
				if (k < 0.5)
					return me.Tween.Easing.Bounce.In(k * 2) * 0.5;
