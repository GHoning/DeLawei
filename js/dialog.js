var dialog = [
	//--------------------------------------------------------------------------
	["kim","intro",
		[	
			"Hallo! Ik ben alex en ik ben gestuurd door mijn schoolkrant om over de vernieuwing te schrijven.",
			"Hallo alex, jouw leraar had al contact met ons opgenomen. Helaas heb ik geen tijd op het moment om jou te helpen maar je bent vrij om rond te lopen en de mensen te interviewen die aanwezig zijn. Als je vragen hebt voor mij wees dan vrij om me lastig te vallen.",
			"Bedankt!"
		]],
		
	["roel","intro", 
		[	"..."			
		]], 
	
	["sam","intro", 
		[	
			"Pardon meneer...",
			"Sorry, kom later anders terug, dan heb ik wel tijd voor je."
		]],

	["tim","intro", 
		[	"..."			
		]], 
		
	//--------------------------------------------------------------------------
	["kim","talk_to_roel",
		[
			"Kunt u mij wat vertellen over de Lawei zelf?",
			"Ja hoor! De Lawei is geopend in 1960. Er waren wat problemen met het bouwen ervan.",
			"Maar nu zijn we 1 van de grootste en bekendste schouwburgen in Nederland!"
		]],
		
	["roel","talk_to_roel", 
		[
			"Pardon meneer, is er misschien iets mis?",
			"Ik heb deze brief gekregen van een goede vriend van mij maar ik kan hem niet helpen. Ik ben nou eenmaal niet meer de jongste.",
			"Ik ben Alex, kan ik misschien ergens mee helpen?"
		]], 
		
	["sam","talk_to_roel", 
		[	"..."			
		]],

	["tim","talk_to_roel", 
		[	"..."			
		]], 
	
	//--------------------------------------------------------------------------
	["kim", "brief_read",
		[
			"Kunt u mij wat vertellen over de Lawei zelf?",
			"Ja hoor! De Lawei is geopend in 1960. Er waren wat problemen met het bouwen ervan.",
			"Maar nu zijn we 1 van de grootste en bekendste schouwburgen in Nederland!"
		]],
		
	["roel", "brief_read",
		[
			"Ik kan u wel helpen hiermee als u dat wilt, meneer…?",
			"Roel, noem me Roel. Het zou veel voor me betekenen als je dat voor me zou willen doen. Maar mijn goede vriend was erg goed in puzzels en raadsels, heb je daar wel het geduld voor jongeman?",
			"Maakt u zich daar maar geen zorgen om, ik zal ervoor zorgen dat u het muziekstuk zult horen zoals hij dat heeft bedoelt.",
			"Bedankt jongeman, heel erg bedankt. Laat me maar weten als ik je ergens mee kan helpen.",
			"Ik geloof dat Tajiri het liefst in het midden van de grote toneelzaal zat. Helaas kan ik me het nummer niet herinneren."
		]], 
		
	["sam","brief_read", 
		[	"..."
		]],

	["tim","brief_read", 
		[	"..."			
		]], 
		
	//--------------------------------------------------------------------------
	["kim", "quest_chair",
		[
			"Heeft u nog iets interessants te vertellen over de Lawei?",
			"Hmm...Wist je dat er een stoel is in de grote toneelzaal die altijd open blijft staan. Ze zeggen dat er een geest op die stoel zit om mee te kijken bij voorstellingen. Wat er ook aan gebeurd ze kunnen het niet repareren."
		]],	
		
		
	["roel", "quest_chair",
		[
			"Waar was de favoriete stoel van Tajiri ook alweer?",
			"In de grote toneelzaal jongeman, helaas weet ik het stoelnummer niet meer maar ik weet zeker dat je er wel uitkomt."
		]],
		
	["sam","quest_chair", 
		[	"Hallo meneer, ik ben Alex en ik ben hier om een artikel te schrijven over de Lawei. Kunt u mij misschien meer vertellen over uzelf en het gebouw?",
			"Ik ben Sam, ik ben 1 van de acteurs voor het komende toneelstuk waar ik eigenlijk niks over mag zeggen.",
			"Zou het mogelijk zijn om een kijkje te nemen in de toneelzaal?",
			"Sorry ik denk dat je daar speciale toestemming voor nodig hebt. Maar laten we een deal maken, als jij mij het nummer van Kim het baliemeisje kan scoren dan wil ik je wel stiekem doorlaten.",
			"Dat is een deal!"			
		]],

	["tim","quest_chair", 
		[	"..."			
		]],
	
	
	//--------------------------------------------------------------------------
	["kim", "get_note1",
		[	"..."
		]],	
		
		
	["roel", "get_note1",
		[
			"..."
		]],
		
	["sam","get_note1", 
		[	"..."
		]],

	["tim","get_note1", 
		[	"..."
		]], 
	
	//--------------------------------------------------------------------------
	["kim", "got_note1",
		[	"..."
		]],	
		
		
	["roel", "got_note1",
		[
			"..."
		]],
		
	["sam","got_note1", 
		[	"..."
		]],

	["tim","got_note1", 
		[	"..."
		]], 
		
	//--------------------------------------------------------------------------
	["kim", "get_number",
		[
			"Kim?",
			"Ja?",
			"Mag ik misschien je nummer?",
			"Waarvoor?",
			"Sam zegt dat hij me wel wilt helpen als ik hem je nummer geef.",
			"Ik zal je mijn nummer geven maar vertel die man dat als hij iets anders van me wilt, het maar zelf moet vragen.",
			"Bedankt! Ik zal de boodschap doorgeven."
		]],	
		
		
	["roel", "get_number",
		[	"..."
		]],
		
	["sam","get_number", 
		[	"..."			
		]],

	["tim","get_number", 
		[	
			"Hallo meneer, ik ben Alex en ik vroeg me af of u tijd heeft voor een interview?",
			"Helaas heb ik nu even geen tijd, kan je later misschien terugkomen?"
		]],
		
	//--------------------------------------------------------------------------
	["kim", "got_number",
		[	"..."
		]],
		
	["roel", "got_number",
		[	"..."
		]],
		
	["sam","got_number", 
		[	
			"Sam, ik heb het nummer voor je.",
			"Geweldig! Bedankt!",
			"Kim heeft gezegd dat als je iets anders van haar wilt het zelf moet vragen.",
			"Haha, ik zal het proberen."
		]],

	["tim","got_number", 
		[	"..."			
		]],
		
	//--------------------------------------------------------------------------
	["kim", "got_note2",
		[	"..."
		]],
		
	["roel", "got_note2",
		[	"..."
		]],
		
	["sam","got_note2", 
		[	"..."			
		]],

	["tim","got_note2", 
		[	
			"Pardon, mag ik misschien uw bar onderzoeken?",
			"Is er een reden dat je mijn bar wilt onderzoeken?",
			"Ik wil graag dit raadsel oplossen voor Roel, hij wuift het briefje voor zich uit waar het raadsel op staat.",
			"Voor Roel… Als jij voor mij een nieuwe kurkentrekker kunt regelen dan mag je mijn bar onderzoeken. Die van mij is vandaag kapot gegaan en ik heb er echt 1 nodig.",
			"Weet u misschien waar ik 1 vandaan zou kunnen halen?",
			"Misschien kan je het aan Kim vragen.",
			"Tot zo!"
		]],
	
	//--------------------------------------------------------------------------
	["kim", "quest_get_pocketknife",
		[	
			"Kim?",
			"Ja Alex?",
			"Weet je misschien waar ik een kurkentrekker vandaan kan halen?",
			"...",
			"Waarvoor heb je een kurkentrekker voor nodig?",
			"Nou… de barman heeft een nieuwe nodig…",
			"Die Tim… hij heeft die kurkentrekker nu al 3 keer vervangen",
			"Misschien kan je Roel vragen, hij loopt wel eens rond met een Zwitsers zakmes."
		]],
		
	["roel", "quest_get_pocketknife",
		[	"..."
		]],
		
	["sam","quest_get_pocketknife", 
		[	"..."			
		]],

	["tim","quest_get_pocketknife", 
		[	"..."			
		]],
	
	//--------------------------------------------------------------------------
	["kim", "quest_get_pocketknife2",
		[	"..."
		]],
		
	["roel", "quest_get_pocketknife2",
		[	
			"Meneer?",
			"Alex, hoe gaat het ermee? Heb je het al gevonden?",
			"Ik ben ermee bezig, maar heeft u misschien een kurkentrekker?",
			"Hmm, ik heb alleen een zakmes, er zit wel een kurkentrekker aan vast, zou dat goed genoeg zijn?",
			"Ja! Helemaal goed.",
			"Krijgt ik hem wel terug?",
			"Bedankt!"
		]],
		
	["sam","quest_get_pocketknife2", 
		[	"..."			
		]],

	["tim","quest_get_pocketknife2", 
		[	"..."			
		]],
	
	//--------------------------------------------------------------------------
	["kim", "have_pocketknife",
		[	"..."
		]],
		
	["roel", "have_pocketknife",
		[	"..."
		]],
		
	["sam","have_pocketknife", 
		[	"..."			
		]],

	["tim","have_pocketknife", 
		[	
			"Hier is de kurkentrekker!",
			"Ah, een zakmes… Ik moet het er maar mee doen. Dankjewel.",
			"Ik mocht hem lenen van Roel.",
			"Nou je bent vrij om mijn bar te onderzoeken. Niet stiekem bier drinken he."
		]],
		
	//--------------------------------------------------------------------------
	["kim", "got_note3",
		[	"..."
		]],
		
	["roel", "got_note3",
		[	"..."
		]],
		
	["sam","got_note3", 
		[	"..."			
		]],

	["tim","got_note3", 
		[	"..."			
		]],
		
	//--------------------------------------------------------------------------
	["kim", "got_note4",
		[	"..."
		]],
		
	["roel", "got_note4",
		[	"..."
		]],
		
	["sam","got_note4", 
		[	"..."			
		]],

	["tim","got_note4", 
		[	"..."			
		]],
		
	//--------------------------------------------------------------------------
	["kim", "got_note5",
		[	"..."
		]],
		
	["roel", "got_note5",
		[	
			"Meneer, ik heb alle delen van het notenschrift.",
			"Dit is echt een kunstwerk van Tajiri. Zullen we naar het toneel gaan om het te spelen?"
		]],
		
	["sam","got_note5", 
		[	"..."			
		]],

	["tim","got_note5", 
		[	"..."			
		]],
	
	
	//kopie doel einden
	//--------------------------------------------------------------------------
	/*["kim", "get_number",
		[	"..."
		]],
		
	["roel", "get_number",
		[	"..."
		]],
		
	["sam","get_number", 
		[	"..."			
		]],

	["tim","get_number", 
		[	"..."			
		]],*/
		
		
	["letter","brief",
		[
			"Beste Roel, Ik weet nog hoe we vroeger samen aan het notenschrift hebben gewerkt en ik heb daar nu nog steeds goede herinneringen aan.",
			"We hebben samen veel plezier gehad maar ik heb het complete notenschrift nooit aan je kunnen laten horen.", 
			"Daar heb ik achteraf best veel spijt van, ik had altijd gedacht dat we genoeg tijd zouden hebben.",
			"Het zou me goed doen als je het notenschrift zou kunnen horen zoals ik het bedoelt heb.",
			"Het eerste stuk zal je vinden in mijn favoriete stoel in de grote toneelzaal.",
			"Mijn goede vriend, ik wens je nog veel goede jaren toe.",
			"Groeten, Tajiri"
		]]
]